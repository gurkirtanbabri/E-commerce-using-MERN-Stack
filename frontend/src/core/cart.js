...
const Cart = (props) => {
  ...
  const check = () => {
    ...
    console.log(order);
    createOrder(order.user, isAuth().token, order)
      .then((data) => {
  ...
  const checkOut = () => (
    <div className="bg-white container p-5 rounded shadow-lg text-dark">
      <h3 className="text-left d-inline">
        Total amount : Rs.<span className="text-success">{total} </span>
      </h3>
      <button
        className="btn btn-lg btn-success float-right text-white "
  ...
  return (
    <Base
      title="Cart Page"
      discription="Get All Your Proucts"
      className="container-fluid bg-dark "
    >
      {products && products.length < 1 && (
        <div className="alert alert-primary">
          <h3 className="text-dark bg-white rounded text-center d-block m-auto p-5">
      ...
      {products && products.length > 0 && checkOut()}
